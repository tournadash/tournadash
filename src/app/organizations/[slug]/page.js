    ...
    <div className="container" style={{ paddingTop: 'var(--space-8)', paddingBottom: 'var(--space-16)' }} id="org-public-profile">
      ...
      {portalNode && createPortal(
        <div className="flex items-center gap-3">
          ...
          <Button variant={followed ? 'secondary' : 'primary'} size="sm" onClick={handleFollow} disabled={!user || members.some(m => m.user_id === user.id)} style={{ marginLeft: '8px' }}>
      ...
      {/* Hero Header Card */}
      <Card className="p-0 mb-8 overflow-hidden relative animate-on-scroll" style={{ border: '2px solid var(--color-border)', borderRadius: 'var(--radius-lg)' }}>
        <div style={{
          position: 'relative',
          width: '100%',
          minHeight: '350px',
          ...
          <div style={{
            ...
            backgroundImage: org.banner_url ? `url(${org.banner_url})` : `url(https://i.imgur.com/w9U8WdE.png)`,
          ...
          <div style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', maxWidth: '800px' }}>
            ...
            {(org.social_discord || org.social_youtube) && (
              <div className="flex gap-4 justify-center flex-wrap" style={{ marginTop: 'var(--space-6)' }}>
                {org.social_discord && (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 'var(--space-4)', marginBottom: 'var(--space-8)' }} className="animate-on-scroll">
            ...
            <Card className="p-4" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <div style={{ fontSize: '10px', fontWeight: '800', color: 'var(--color-text-muted)', letterSpacing: '0.05em' }}>TOURNAMENTS HOSTED</div>
              </div>
              <div className="dash-stat-value" style={{ fontSize: 'var(--text-lg)', color: '#3cc83c', lineHeight: '1' }}>{tournamentsHosted}+</div>
            </Card>

            <Card className="p-4" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <div style={{ color: 'var(--color-text-secondary)', display: 'flex' }}>
                  <svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" strokeWidth="2" fill="none"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                </div>
                <div style={{ fontSize: '10px', fontWeight: '800', color: 'var(--color-text-muted)', letterSpacing: '0.05em' }}>COMPETITORS REGISTERED</div>
              </div>
              <div className="dash-stat-value" style={{ fontSize: 'var(--text-lg)', color: '#38bdf8', lineHeight: '1' }}>{competitorsRegistered.toLocaleString()}+</div>
            </Card>

            <Card className="p-4" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <div style={{ color: 'var(--color-text-secondary)', display: 'flex' }}>
                  <svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" strokeWidth="2" fill="none"><circle cx="12" cy="8" r="7"/><polyline points="8.21 13.89 7 23 12 20 17 23 15.79 13.88"/></svg>
                </div>
                <div style={{ fontSize: '10px', fontWeight: '800', color: 'var(--color-text-muted)', letterSpacing: '0.05em' }}>PRIZES DISTRIBUTED</div>
              </div>
              <div className="dash-stat-value" style={{ fontSize: 'var(--text-lg)', color: '#facc15', lineHeight: '1' }}>{prizesDistributed.toLocaleString()}rs+</div>
            </Card>

            <Card className="p-4" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <div style={{ color: 'var(--color-text-secondary)', display: 'flex' }}>
                  <svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" strokeWidth="2" fill="none"><path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.5 3.8 17 5 19 5a1 1 0 0 1 1 1z"/></svg>
                </div>
                <div style={{ fontSize: '10px', fontWeight: '800', color: 'var(--color-text-muted)', letterSpacing: '0.05em' }}>ACTIVE SERVERS</div>
              </div>
              <div className="dash-stat-value" style={{ fontSize: 'var(--text-lg)', color: '#3cc83c', lineHeight: '1' }}>{activeServers} ONLINE</div>
            </Card>
        ...
        <div className="flex flex-col gap-8">
          ...
          <Card className="p-6">
            <div style={{
              ...
              <h3 style={{ fontSize: 'var(--text-xl)', color: '#3cc83c', margin: 0, fontWeight: '800' }}>
                🏆 FEATURED EVENTS
              </h3>
            ...
            {tournaments.length > 0 ? (
              <div className="featured-events-grid">
                {tournaments.filter(t => t.status !== 'ENDED').slice(0, 2).map((t) => (
